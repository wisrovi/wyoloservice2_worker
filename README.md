...
### Version 2.2.18 (Current Release) - 2026-08-06
*   **Corporate Branding Media Assets:** Added the actual branding images (`wtrain.jpg`, `wpipe.jpg`, and `logo.jpg`) under `wtrain/app/media/` so the DOCX reports generated by the EDA and LLM analyzers resolve their `/app/media` references at runtime. The images are now part of the Docker build context.
*   **LLM Analyzer Refactor:** Moved the `python-docx` imports to module level in `llm_analyzer.py`, eliminating per-run import overhead and keeping the DOCX generation logic consistent with the EDA analyzer.
*   **Code Formatting:** Reflowed `dataset_analyzer.py` and `llm_analyzer.py` with Black to standardize line lengths and whitespace, with no behavior changes.
*   **Version Update:** Bumped executor version to v2.2.18.

### Version 2.2.17 - 2026-08-06
